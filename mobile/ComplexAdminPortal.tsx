...
import ConfirmedReservationsReport from './ConfirmedReservationsReport';
import { SportsAction, SportsHero, SportsSectionTitle } from './SportsBrand';

const API_URL = process.env.EXPO_PUBLIC_API_URL || 'http://localhost:8000';
type Props = { styles: any; onLogout: () => void };
type Section = 'dashboard' | 'settings' | 'fields' | 'calendar' | 'payments' | 'confirmedReservations' | 'reports';
...
export default function ComplexAdminPortal({ styles, onLogout }: Props) {
  ...
  function renderSection() {
    ...
    if (activeSection === 'confirmedReservations') return <ConfirmedReservationsReport styles={styles} selectedComplex={selectedComplex} />;
