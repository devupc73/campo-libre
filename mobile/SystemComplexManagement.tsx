...
import { ComplexMediaEditor } from './ComplexMedia';
...
export default function SystemComplexManagement({ styles, systemAdminId }: any) {
  ...
  async function loadData() {
    try {
      ...
      setComplexes(await complexResponse.json()); setUsers(await userResponse.json());
    } catch (error: any) { setMessage(error.message || 'No se pudo cargar complejos o usuarios.'); }
  }
  useEffect(() => { loadData(); }, []);

  function fillComplex(complex: any) {
    setSelectedComplex(complex); setComplexName(complex?.name || ''); setComplexAddress(complex?.address || '');
    setComplexLat(String(complex?.latitude ?? '-12.0464')); setComplexLng(String(complex?.longitude ?? '-77.0428'));
    setComplexAdminId(complex?.complex_admin_user_id ? String(complex.complex_admin_user_id) : '');
    setImageUrl(complex?.image_url || ''); setLogoUrl(complex?.logo_url || '');
  }

  async function saveAssignment(complexId: number) {
    if (!complexAdminId) return;
    const response = await fetch(`${API_URL}/complex-admin/assignments`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ complex_id: complexId, admin_user_id: Number(complexAdminId) }) });
  ...
  async function saveComplex(update = false) {
    ...
    try {
      const payload = { name: complexName, address: complexAddress, latitude: Number(complexLat), longitude: Number(complexLng), system_admin_user_id: Number(systemAdminId), complex_admin_user_id: complexAdminId ? Number(complexAdminId) : null, description: selectedComplex?.description || '', phone: selectedComplex?.phone || '', image_url: imageUrl || null, logo_url: logoUrl || null, rating: selectedComplex?.rating || 0 };
      ...
      if (!response.ok) throw new Error(await responseMessage(response));
      const data = await response.json(); await saveAssignment(data.id); fillComplex(data); await loadData(); setMessage('Complejo guardado correctamente.');
    } catch (error: any) { setMessage(error.message || 'No se pudo guardar el complejo.'); }
  }

  async function changeStatus(status: 'active' | 'inactive') {
    if (!selectedComplex) return setMessage('Selecciona un complejo.');
    const response = await fetch(`${API_URL}/sports-complexes/${selectedComplex.id}/status`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status, system_admin_user_id: Number(systemAdminId) }) });
    if (!response.ok) return setMessage(await responseMessage(response));
    const updated = await response.json(); fillComplex(updated); await loadData(); setMessage(status === 'inactive' ? 'Complejo desactivado.' : 'Complejo reactivado.');
  }

  const selectedStatus = String(selectedComplex?.status || 'active').toLowerCase();
  return <View>
    <Text style={styles.title}>Gestión de complejos</Text>
    <Text style={styles.subtitle}>Crea complejos, asigna administradores y administra su identidad visual.</Text>
    <ComboSelect styles={styles} label="Seleccionar complejo existente" value={selectedComplex ? String(selectedComplex.id) : ''} options={complexes.map((item) => ({ label: `${item.name} · ${String(item.status || 'active').toLowerCase() === 'inactive' ? 'INACTIVO' : 'ACTIVO'} (${item.address})`, value: String(item.id) }))} onChange={(value) => fillComplex(complexes.find((item) => String(item.id) === value))} />
    ...
    <ComplexMediaEditor styles={styles} imageUrl={imageUrl} logoUrl={logoUrl} onImageChange={setImageUrl} onLogoChange={setLogoUrl} />
    <ComboSelect styles={styles} label="Administrador del complejo" value={complexAdminId} options={users.filter((user) => user.role === 'complex_admin').map((user) => ({ label: `${user.full_name} (${user.email})`, value: String(user.id) }))} onChange={setComplexAdminId} />
