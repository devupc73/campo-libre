...
export default function SystemComplexManagement({ styles, systemAdminId }: any) {
  ...
  async function saveComplex(update = false) {
    ...
    try {
      const payload = {
        ...
        system_admin_user_id: Number(systemAdminId),
        complex_admin_user_id: complexAdminId ? Number(complexAdminId) : null,
        description: selectedComplex?.description || '',
        phone: selectedComplex?.phone || '',
        image_url: selectedComplex?.image_url || '',
        rating: selectedComplex?.rating || 0,
      };

      const url = update && selectedComplex
        ? `${API_URL}/sports-complexes/${selectedComplex.id}`
        : `${API_URL}/sports-complexes`;
  ...
  return (
    <View>
      ...
      <Text style={styles.subtitle}>Crea complejos y asigna administradores operativos.</Text>

      <ComboSelect
        styles={styles}
        label="Seleccionar complejo existente"
        value={selectedComplex ? String(selectedComplex.id) : ''}
        options={complexes.map((item) => ({
          label: `${item.name} (${item.address})`,
          value: String(item.id),
        }))}
        onChange={(value) => {
          const complex = complexes.find((item) => String(item.id) === value);
          fillComplex(complex);
        }}
      />
      ...
      <ComboSelect
        styles={styles}
        label="Administrador del complejo"
        value={complexAdminId}
        options={users
          .filter((user) => user.role === 'complex_admin')
          .map((user) => ({
      ...
      <TouchableOpacity style={styles.primaryButton} onPress={() => saveComplex(false)}>
        <Text style={styles.buttonText}>Crear complejo</Text>
      </TouchableOpacity>

      <TouchableOpacity style={styles.secondaryButton} onPress={() => saveComplex(true)}>
        <Text style={styles.buttonText}>Actualizar complejo seleccionado</Text>
      </TouchableOpacity>
