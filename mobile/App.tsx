...
export default function App() {
  ...
  async function registerPublic() {
    ...
    try {
      const response = await fetch(`${API_URL}/auth/register`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ full_name: fullName, email, phone, bank_account: bankAccount, interbank_account: interbankAccount, role, password }),
      });
      if (!response.ok) throw new Error();
  ...
  if (portal === 'home') {
    ...
    content = <View style={styles.card}><TouchableOpacity style={styles.secondaryButton} onPress={goHome}><Text style={styles.buttonText}>Volver a inicio</Text></TouchableOpacity><ComplexAdminPortal styles={styles} onLogout={logout} /></View>;
