...
type Screen = 'home' | 'login' | 'register' | 'dashboard' | 'courts' | 'reservations' | 'matches' | 'wallet';
...
export default function App() {
  ...
  async function loadCount(path: string, label: string) {
    setResult(`Consultando ${label}...`);
    try {
      const response = await fetch(`${API_URL}${path}`);
      const data = await response.json();
      setResult(`${label}: ${Array.isArray(data) ? data.length : 0} registros encontrados.`);
    } catch {
      setResult(`No se pudo consultar ${label}.`);
    }
  }
