...
export default function CaptainOfficialAssociation({ styles, userId, selectedMatch, onSaved }: any) {
  ...
  async function loadComplexes() {
    try {
      ...
      setMessage(error.message || 'No se pudieron cargar los complejos.');
    }
  }
  ...
  async function loadSchedules(courtId: string, preserveScheduleId = '') {
    ...
    try {
      ...
      setSchedules(active);
      if (preserveScheduleId && active.some((slot: any) => String(slot.id) === preserveScheduleId)) {
        setSelectedScheduleId(preserveScheduleId);
      }
    } catch (error: any) {
      setSchedules([]);
      setMessage(error.message || 'No se pudieron cargar las franjas.');
    }
  }
  ...
  async function saveAssociation() {
    ...
    try {
      ...
      setMessage(error.message || 'No se pudo guardar la asociación oficial.');
    }
  }
